...
class ClassAccessHelperTest {
	...
	def test(Integer offset = 0){
		...
		classFile.constantPool = ['TestClass','Object', 'a', 'b','test(0)','foo(1)', 'native(0)']
		classFile.fields = [2,3]

		def method1 = new PrecompiledMethod()
		method1.signatureIndex = 4
		...
		def constPoolSize = classFile.constantPool.sum{
			it.length() + Heap.SLOT_SIZE
		}
