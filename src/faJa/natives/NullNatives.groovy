...
class NullNatives {
	...
	static isNull = { StackFrame stackFrame, Heap heap, ClassLoader classLoader ->
		...
		Integer boolPtr = heap.createBool(classLoader.findClass(heap, Compilator.BOOL_CLASS), BoolNatives.TRUE)
	...
	static toS = { StackFrame stackFrame, Heap heap, ClassLoader classLoader ->
		stackFrame.methodStack.pop()
		Integer strPtr = heap.createString(classLoader.findClass(heap, Compilator.STRING_CLASS), Compilator.NULL_KEYWORD) //
